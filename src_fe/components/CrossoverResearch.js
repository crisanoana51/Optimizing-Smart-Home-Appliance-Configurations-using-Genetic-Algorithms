...
const CrossoverResearch = () => {
    ...
    const handleSearch = async () => {
        try {
            const response = await axios.post('http://localhost:5000/compare_crossover', {
                types: selectedTypes,
    ...
    return (
        <div className={styles.respage}>
            <header className="header">
                ...
                <h1>Crossover Methods Global Fitness Comparison</h1>
                ...
                <div className={styles.chartSection } ref={chartSectionRef}>
                    {fitnessHistories && Object.keys(fitnessHistories).length > 0 ? (
                        <Line
                            ref={chartRef}
                            data={{
                                labels: Array.from({ length: fitnessHistories.tournament_selection.length  }, (_, i) => i + 1),
                                datasets: [
                                    {
                                        label: 'Double-point Crossover',
                                        data: fitnessHistories.tournament_double,
                                        borderColor: 'red',
                                        fill: false,
                                    },
                                    {
...
export default CrossoverResearch;
